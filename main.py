...
Original file is located at
    https://colab.research.google.com/drive/1EbaqATHxJHDjWdr2u2axOBR88eVu5pw8
"""
...
Original file is located at
    https://colab.research.google.com/drive/1eINLdLj1z4Ic_tcO6Kh0L7-Te7grHLja
...
Version 2 — Upgraded with:
  1. Random Undersampling (RUS) pipeline competing head-to-head with SMOTE
  2. PCA 'Crime Scene' diagnostic plot of synthetic SMOTE samples
  3. Threshold sensitivity analysis: m_p < 0.5 / 1.0 / 2.0
  4. F2-Score (beta=2) as primary ranking metric throughout (recall-weighted)
  5. Overfitting diagnostic checks:
       - Train vs validation F2-Score gap per pipeline per fold
       - Per-fold F2-Score variance (stability)
       - Hyperparameter range boundary check
       - Learning curve (Fig8) for Pipeline 1

Note on cross-database generalisation:
  AFLOWLIB was considered but excluded because its publicly accessible data
  does not consistently expose BoltzTraP-derived transport features (S_n, PF_n,
  m_n) required by this model. The Ricci et al. (2017) Scientific Data dataset
  (doi: 10.1038/sdata.2017.85) is the recommended external validation set --
  it uses the same BoltzTraP methodology on ~48,000 materials and exposes the
  same feature columns. Add 'ricci_data.csv' to the project folder to enable
  that test in a future revision.

Literature basis:
  - Chawla et al. (2002) JAIR -- original SMOTE paper; recommends visualising synthetic samples
  - Drummond & Holte (2003) ICML -- canonical RUS vs SMOTE comparison
  - He & Garcia (2009) IEEE TKDE -- comprehensive imbalance learning review
  - Hautier et al. (2013) Nat. Chem. -- m_p thresholds for p-type TCO candidates
  - Ricci et al. (2017) Sci. Data -- BoltzTraP transport dataset (recommended for future transfer test)
"""
...
import anthropic    #|  set ANTHROPIC_API_KEY env var
...
try:
    ...
    save_dir = '/content/drive/MyDrive/ml_project_v6_Ens_Ext_No_SMOTE'
    print(f"Google Drive mounted. Outputs -> {save_dir}")
except ModuleNotFoundError:
    save_dir = 'ml_project'
    print(f"Running locally. Outputs -> {save_dir}")
...
DRIVE_ROOT = '/content/drive/MyDrive/ml_project_v6_Ens_Ext_No_SMOTE'
df = pd.read_csv(os.path.join(DRIVE_ROOT, 'boltztrap_mp.csv'))
...
#    Pipeline 6 -- RUS replaces SMOTE (Drummond & Holte 2003)
#    RUS sampling_strategy='auto' down-samples majority to match minority count.
# =============================================================================
...
df_csv.to_csv(os.path.join(save_dir, 'Ablation_Study_Metrics.csv'))
...
df_gap.to_csv(os.path.join(save_dir, 'Overfitting_TrainVal_Gap.csv'), index=False)
...
df_var.to_csv(os.path.join(save_dir, 'Overfitting_FoldVariance.csv'), index=False)
...
if full_imp_records:
    ...
    imp_summary.to_csv(os.path.join(save_dir, 'Feature_Importances.csv'))
...
plt.tight_layout()
p = os.path.join(save_dir, 'Fig1_Confusion_Matrices.pdf')
...
plt.tight_layout()
p = os.path.join(save_dir, 'Fig2_ROC_PR_Curves.pdf')
plt.savefig(p, dpi=600, bbox_inches='tight', format='pdf')
plt.close()
print(f"  Saved: {p}")
...
plt.tight_layout()
p = os.path.join(save_dir, 'Fig3_Radar_Chart.pdf')
plt.savefig(p, dpi=600, bbox_inches='tight', format='pdf')
plt.close()
print(f"  Saved: {p}")
...
if full_imp_records:
    ...
    plt.tight_layout()
    p = os.path.join(save_dir, 'Fig4_Feature_Importances.pdf')
    plt.savefig(p, dpi=600, bbox_inches='tight', format='pdf')
    plt.close()
    print(f"  Saved: {p}")
...
plt.tight_layout()
p = os.path.join(save_dir, 'Fig8_Learning_Curve.pdf')
plt.savefig(p, dpi=600, bbox_inches='tight', format='pdf')
plt.close()
print(f"  Saved: {p}")
...
lc_df.to_csv(os.path.join(save_dir, 'Learning_Curve_Data.csv'), index=False)
...
plt.tight_layout()
p = os.path.join(save_dir, 'Fig5_PCA_SMOTE_CrimeScene.pdf')
plt.savefig(p, dpi=600, bbox_inches='tight', format='pdf')
plt.close()
print(f"  Saved: {p}")
...
#    m_p < 0.5 / 1.0 / 2.0. Three metrics reported: F1, F2, MCC.
#    F2 (primary) is used to determine ranking robustness.
#    scoring='f1' is intentionally used inside RandomizedSearchCV here.
#    Rationale: tuning on F2 or recall directly collapses the inner search
#    toward degenerate high-recall/low-precision solutions, inflating the
#    outer-fold F2 scores artifactually. F1 is the correct internal criterion
#    because it keeps the model in a reasonable precision/recall operating
#    region. F2 is reserved for external pipeline comparison only.
# =============================================================================
print("\n" + "="*60)
...
sensitivity_pipelines = {
    ...
    'Baseline (No resampling)': lambda: ImbPipeline([
        ('imputer', SimpleImputer(strategy='median')),
        ('fs',      SelectKBest(score_func=partial(mutual_info_classif, random_state=42), k=3)),
        ('scaler',  StandardScaler()),
        ('xgb',     XGBClassifier(eval_metric='logloss', random_state=42)),
...
for thresh in THRESHOLDS:
    y_thresh = (df['m_p'] < thresh).astype(int)
    minority_pct = y_thresh.mean() * 100
    imb = (y_thresh == 0).sum() / max((y_thresh == 1).sum(), 1)

    print(f"\n  Threshold m_p < {thresh}:  "
          f"minority={y_thresh.sum()} ({minority_pct:.1f}%),  "
          f"imbalance ratio={imb:.1f}:1")

    for pipe_name, pipe_factory in sensitivity_pipelines.items():
        fold_f1s, fold_f2s, fold_mccs = [], [], []

        for fold, (tr_idx, te_idx) in enumerate(skf.split(X, y_thresh)):
            X_tr, X_te = X.iloc[tr_idx], X.iloc[te_idx]
            y_tr, y_te = y_thresh.iloc[tr_idx], y_thresh.iloc[te_idx]

            if y_tr.sum() < 5:
                print(f"    Fold {fold+1}: too few minority samples "
                      f"({y_tr.sum()}) -- skipping fold.")
                continue

            pipe = pipe_factory()
            search = RandomizedSearchCV(
                pipe, xgb_param_dist, n_iter=15, cv=3,
            ...
            try:
                ...
                fold_f2s.append(fbeta_score(y_te, preds, beta=2, zero_division=0))
                fold_mccs.append(matthews_corrcoef(y_te, preds))
            except Exception as e:
                print(f"    Fold {fold+1} failed for {pipe_name}: {e}")

        if fold_f1s:
            sens_records.append({
                'Threshold':  f'm_p < {thresh}',
                'Minority %': f'{minority_pct:.1f}%',
                'Imbalance':  f'{imb:.1f}:1',
                'Pipeline':   pipe_name,
                'F1-Score':   f'{np.mean(fold_f1s):.3f} +/- {np.std(fold_f1s):.3f}',
                'F2-Score':   f'{np.mean(fold_f2s):.3f} +/- {np.std(fold_f2s):.3f}',
                'MCC':        f'{np.mean(fold_mccs):.3f} +/- {np.std(fold_mccs):.3f}',
                'F1_mean':    np.mean(fold_f1s),
                'F2_mean':    np.mean(fold_f2s),
...
cols_display = ['Threshold', 'Minority %', 'Imbalance', 'Pipeline', 'F1-Score', 'F2-Score', 'MCC']
print(df_sens[cols_display].to_string(index=False))

df_sens[cols_display].to_csv(
    os.path.join(save_dir, 'Threshold_Sensitivity.csv'), index=False)
print(f"\n  Saved: Threshold_Sensitivity.csv")

# Fig 6: three-panel bar chart -- F1, F2 (primary), MCC
pipe_names    = list(sensitivity_pipelines.keys())
thresh_labels = [f'm_p < {t}' for t in THRESHOLDS]
n_pipes = len(pipe_names)
x_pos   = np.arange(len(THRESHOLDS))
width   = 0.25
sens_colors = ['#d7191c', '#7b2d8b', '#fdae61']

fig, axes = plt.subplots(1, 3, figsize=(18, 6))

for ax_idx, (metric, label) in enumerate(
    [('F1_mean', 'F1-Score (secondary)'),
     ('F2_mean', r'F2-Score ($\beta=2$, PRIMARY)'),
     ('MCC_mean', 'MCC (secondary)')]):

    ax = axes[ax_idx]
    for i, (pipe_name, color) in enumerate(zip(pipe_names, sens_colors)):
        vals = []
        for thresh in THRESHOLDS:
            row = df_sens[
                (df_sens['Pipeline'] == pipe_name) &
                (df_sens['Threshold'] == f'm_p < {thresh}')
            ]
            vals.append(row[metric].values[0] if len(row) > 0 else 0.0)

        offset = (i - n_pipes / 2 + 0.5) * width
        ax.bar(x_pos + offset, vals, width, label=pipe_name,
    ...
    ax.set_title(f'Threshold Sensitivity: {label}\n'
                 r'(Effect of $m_p$ cutoff on resampling strategy ranking)',
...
plt.tight_layout()
p = os.path.join(save_dir, 'Fig6_Threshold_Sensitivity.pdf')
plt.savefig(p, dpi=600, bbox_inches='tight', format='pdf')
plt.close()
print(f"  Saved: {p}")
...
df_extra_csv.to_csv(os.path.join(save_dir, 'Extra_Pipeline_Results_NO RESAMPLING.csv'))
...
p = os.path.join(save_dir, 'Fig7_Ensemble_Comparison_NO RESAMPLING.pdf')
plt.savefig(p, dpi=600, bbox_inches='tight', format='pdf')
p_tiff = os.path.join(save_dir, 'Fig7_Ensemble_Comparison_NO RESAMPLING.tiff')
...
df_extra_csv.to_csv(os.path.join(save_dir, 'Extra_Pipeline_Results.csv'))
...
plt.tight_layout(pad=0.5)
p = os.path.join(save_dir, 'Fig7_Ensemble_Comparison.pdf')
plt.savefig(p, dpi=600, bbox_inches='tight', format='pdf')
p_tiff = os.path.join(save_dir, 'Fig7_Ensemble_Comparison.tiff')
plt.savefig(p_tiff, dpi=300, bbox_inches='tight', format='tiff')
plt.close()
print(f"  Saved: {p}")
print(f"  Saved: {p_tiff}")
...
# =============================================================================
# 10. LLM API EXPLANATION MODULE
# =============================================================================
import os
os.environ['ANTHROPIC_API_KEY'] = 'sk-ant-...'

print("\n" + "="*60)
print("  LLM API EXPLANATION MODULE")
print("="*60)
...
if not ANTHROPIC_API_KEY:
    ...
    print("  Set it and rerun only Section 10 to generate LLM_Explanations.txt")
    print("  (Sections 1-9 do not need to be rerun -- all figures already saved.)")
else:
    print("\n  [1/4] Fitting Full Pipeline (Pipeline 1) on entire dataset...")

    llm_pipeline = ImbPipeline([
        ('imputer', SimpleImputer(strategy='median')),
        ('fs',      SelectKBest(score_func=partial(mutual_info_classif, random_state=42), k=3)),
        ('scaler',  StandardScaler()),
        ('smote',   BorderlineSMOTE(random_state=42, kind='borderline-1')),
    ...
    llm_pipeline.fit(X, y)

    print("  [2/4] Computing predicted probabilities for all materials...")
    probas = llm_pipeline.predict_proba(X)[:, 1]
    preds  = llm_pipeline.predict(X)

    rank_df = X[PHYSICAL_FEATURES].copy()
    rank_df['y_true']    = y.values
    rank_df['y_pred']    = preds
    rank_df['proba_pos'] = probas
    rank_df['is_tp']     = ((rank_df['y_pred'] == 1) & (rank_df['y_true'] == 1)).astype(int)
    top5 = rank_df.sort_values(['is_tp', 'proba_pos'], ascending=[False, False]).head(5)

    print("  [3/4] Calling Anthropic API for top-5 candidates...")
    client = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)
    explanations = []

    for rank, (idx, row) in enumerate(top5.iterrows(), start=1):
        label   = "True Positive"  if row['is_tp'] == 1 else "False Positive"
        proba   = row['proba_pos']
        feat_str = "\n".join(
            f"  - {f}: {row[f]:.4f}" for f in PHYSICAL_FEATURES
        )

        prompt = (
            "You are an expert computational materials scientist specialising "
            "in transparent conducting oxides (TCOs) and machine learning-assisted "
            "materials discovery.\n\n"
            "A machine learning model (XGBoost + Borderline-SMOTE, trained on the "
            "BoltzTraP/Materials Project database) has predicted the following "
            f"material as a high-mobility p-type semiconductor candidate with "
            f"probability {proba:.3f} (threshold: m*_h < 1.0 m_e).\n\n"
            "The model used only n-type transport properties as input features "
            "(to avoid physics-level data leakage from the Boltzmann Transport "
            "Equation):\n"
            f"{feat_str}\n\n"
            "Task: In 3-4 concise sentences suitable for a journal Results section, explain:\n"
            "1. What each feature value physically means for this material.\n"
            "2. Why the combination of these n-type values makes it a plausible "
            "p-type high-mobility candidate (cross-band-structure hypothesis).\n"
            "3. What experimental or DFT follow-up would be needed to confirm it.\n\n"
            "Write in a scientific but accessible style. Do not use bullet points. "
            "Do not mention XGBoost, SMOTE, or any model mechanics -- focus on physics."
        )

        print(f"     Candidate {rank}/5  (index={idx}, P={proba:.3f}, {label})")
        try:
            ...
            explanation_text = msg.content[0].text
        except Exception as e:
            explanation_text = f"[API call failed: {e}]"

        explanations.append({
            'rank': rank, 'dataset_index': idx, 'label': label,
            'proba': proba, 'features': {f: row[f] for f in PHYSICAL_FEATURES},
            'explanation': explanation_text,
        })
        time.sleep(0.5)

    print("  [4/4] Writing LLM_Explanations.txt...")
    out_path = os.path.join(save_dir, 'LLM_Explanations.txt')
    with open(out_path, 'w', encoding='utf-8') as f:
        f.write("LLM-GENERATED EXPLANATIONS OF TOP PREDICTED CANDIDATES\n")
        f.write("Model: XGBoost + Borderline-SMOTE (Full Pipeline, Pipeline 1)\n")
        f.write(f"LLM:   {LLM_MODEL}\n")
        f.write("="*60 + "\n\n")
        for entry in explanations:
            f.write(
                f"Candidate #{entry['rank']}  "
                f"(Dataset index: {entry['dataset_index']}, "
                f"P(minority)={entry['proba']:.3f}, {entry['label']})\n"
            )
            f.write("-"*50 + "\n")
...
Usage
-----
Call run_all_tests(oof_preds, fold_metrics, y, save_dir=save_dir)
...
run_all_tests(oof_preds, fold_metrics, y, save_dir=save_dir)